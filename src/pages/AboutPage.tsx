...
import { aboutTimelineService, resolveTimelineImage } from "@/lib/aboutTimelineService";
...
export function AboutPage() {
  ...
  const { data: timelineRemote } = useQuery({
    queryKey: ["about-timeline-public"],
    queryFn: () => aboutTimelineService.getActiveItems(),
  });

  const TIMELINE_ITEMS = useMemo(() => {
    const base = timelineRemote ?? DEFAULT_ABOUT_TIMELINE_ITEMS;
    return base.map((item) => ({
      year: item.year,
      img: resolveTimelineImage(item.img_path, imgBase),
      events: item.events,
    }));
  }, [timelineRemote, imgBase]);
