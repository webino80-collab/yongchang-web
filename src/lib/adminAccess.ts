...
export const SUPER_ADMIN_LEVEL = 10;
export const SUPER_ADMIN_ONLY_PATHS = ["/admin/renewal-alerts"] as const;
...
const SUPER_ADMIN_PATH_SET = new Set<string>(SUPER_ADMIN_ONLY_PATHS);
...
export function isSuperAdmin(profile: Profile | null | undefined): boolean {
  return profile?.is_admin === true && profile.level >= SUPER_ADMIN_LEVEL;
}
...
export function isSuperAdminOnlyPath(pathname: string): boolean {
  return SUPER_ADMIN_PATH_SET.has(normalizeAdminPath(pathname));
}
